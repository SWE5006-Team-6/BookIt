...
export const options = {
  ...
  thresholds: {
    ...
    'http_req_failed{scenario:bookings_check_in}': ['rate<0.01'],
    ...
    'http_req_duration{scenario:bookings_create}': ['p(95)<2000'],
    'http_req_duration{scenario:bookings_check_in}': ['p(95)<2000'],
    'http_req_duration{scenario:bookings_cancel}': ['p(95)<2000'],
  },
};
...
assertFixtureCapacity(
  'bookings_check_in',
  checkInFixtures,
  calculateIterationsNeeded(options.scenarios.bookings_check_in),
);
assertFixtureCapacity(
  'bookings_cancel',
...
function loadManifest() {
  try {
    ...
    throw new Error(
      `Failed to load performance manifest at "${MANIFEST_PATH}": ${String(error)}`,
    );
  }
}
...
function assertFixtureCapacity(name, fixtures, requiredCount) {
  if (fixtures.length < requiredCount) {
    throw new Error(
      `Scenario "${name}" requires ${requiredCount} fixtures but only ${fixtures.length} were seeded.`,
    );
  }
}
...
function getScenarioFixture(fixtures) {
  ...
  if (index >= fixtures.length) {
    throw new Error(
      `Scenario "${exec.scenario.name}" exceeded seeded fixture count at iteration ${index}.`,
    );
  }
...
export function cancelBooking() {
  ...
  const response = http.post(
    ...
    JSON.stringify({
      reason: `Performance cancellation ${exec.scenario.iterationInTest + 1}`,
    }),
    requestParams(fixture.headers, true),
  );
  const payload = parseJsonBody(response);
